...
新建账目时，数据库同时保存创建时间和修改时间；编辑账目只更新修改时间。顶部的“显示记录时间”默认不勾选，勾选后表格才显示“创建时间”和“修改时间”两列。显示时间列时，CSV 导出也会包含这两列。

程序启动时会自动为旧数据库补充 `created_at` 和 `updated_at` 字段，不删除或改写原有账目。旧账目没有可靠的历史时间，因此显示为“未知”；旧账目以后被编辑时会记录新的修改时间，但创建时间仍保持未知。
